...
import com.interviewspringboot.utils.LlmPromptConstants;
...
public class AIServiceImpl implements AIService {
        ...
        if (messages.isEmpty()) {
            ...
            for (int i = 0; i < sampleSize; i++) {
                ...
                log.info("示例消息 {}: role={}, timestamp={}, content={}...",
                        ...
                        msg.getContent() != null && msg.getContent().length() > 30 ? msg.getContent().substring(0, 30)
                                : msg.getContent());
            }
        }
    ...
    public SseEmitter streamChat(Long userId, String sessionId, String message) {
        ...
        try {
            ...
            // 获取会话上下文
            MessageManager messageManager = getOrCreateMessageManager(sessionId, userId, interview);
                ...
                try {
                    ...
                    resultFlowable.blockingForEach(generationResult -> {
                        if (generationResult.getOutput() != null &&
                            ...
                            String contentChunk = generationResult.getOutput().getChoices().get(0).getMessage()
                                    .getContent();
                            ...
                            if (contentChunk != null) {
                                ...
                                StreamResponse streamResponse = new StreamResponse("message", contentChunk,
                                        isLastChunk);
                                try {
                    ...
                    try {
                        ...
                        log.error("完成emitter出错 (after stream error): sessionId={}, error={}", sessionId,
                                ex.getMessage());
                    }
                }
            });
    ...
    private MessageManager getOrCreateMessageManager(String sessionId) {
        ...
        if (sessionId != null && sessionId.startsWith("default-session-")) {
            ...
            // 使用PromptComposer构建提示词
            promptComposer.addBaseRule(LlmPromptConstants.BASE_RULES);
            promptComposer.addPersona(LlmPromptConstants.INTERVIEW_SESSION_CHAT);

            // 注意：此处无法获取resume信息，因为没有userId和interview
            // 原代码可能存在问题，这里暂时移除这部分逻辑
            // Result<Resume> resumeResult = resumeService.getResumeDetail(userId, interview.getResumeId());
            
            // 组合提示词
            String composedPrompt = promptComposer.compose();
    ...
    /**
     * 获取或创建消息管理器（重载，带userId和interview参数）
     * @param sessionId 会话ID
     * @param userId 用户ID
     * @param interview 面试对象
     * @return 消息管理器
     */
    private MessageManager getOrCreateMessageManager(String sessionId, Long userId, Interview interview) {
        // 检查是否已有该默认会话上下文
        if (sessionContexts.containsKey(sessionId)) {
            return sessionContexts.get(sessionId);
        }
        ...
        // 获取简历信息作为上下文，直接拼接Resume对象
        if (interview != null) {
            Result<Resume> resumeResult = resumeService.getResumeDetail(userId, interview.getResumeId());
            if (resumeResult.getCode() == 200 && resumeResult.getData() != null) {
                Resume resume = resumeResult.getData();
                log.info("resume" + resume);
                promptComposer.addContext("上下文： \n" + resume.toString());
            }
        }
        // 组合提示词
        String composedPrompt = promptComposer.compose();
     ...
     */
    @Override
    public Result<Map<String, Object>> saveConversation(Long userId, String sessionId,
     ...
     * 生成面试总结
     */
    private String generateInterviewSummary(Interview interview, InterviewRound round,
